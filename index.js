...
//==============		
    ...
    case 'help': case 'menu': case 'helep':
          ...
          let hasil = `${res[0].isi}\n~${res[0].by}`
          menu =`# مرحبا ${pushname}
• المستخدم عضوية : ${premi}
• حد : ${isOwner ? 'Unlimited' : isPremium ? 'Unlimited' : getLimit(sender, limitCount, limit)}
• النقاط : $${(getBalance(sender, balance))}
• المالك : ${Miminnya}
• البوت اسم  : ${BotName}
• الوضع : ${publik ? 'Public' : 'Self'}
• الوقت : ${runtime(process.uptime())}
• شات : ${totalchat.length} Chat
• آل vip : ${pendaftar.length} User
• رسائل : ${hit_today.length} Hit
• كافة الرسائل : ${totalhit} Hit
• المحرك : ${modern.browserDescription[1]}
• السرفر : ${modern.browserDescription[0]}
• النسخة : ${modern.browserDescription[2]}
• الجهاز : IPhone 13 
• السرعة :  ${latensi.toFixed(4)} Second
• النسخة : ${modern.user.phone.wa_version}`
